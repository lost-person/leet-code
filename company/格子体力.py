...
class MinHeap:
    ...
    def __sink(self, index):
        ...
        while index << 1 <= self.cnt:
            tmp_index = index << 1
            if tmp_index + 1 <= self.cnt and self.data[
                    tmp_index + 1].dist < self.data[tmp_index].dist:
                tmp_index += 1
...
while not min_heap.empty():
    ...
    for direction in directions:
        ...
        if 0 <= new_x < m and 0 <= new_y < n and dp[new_x][
                new_y] > node.dist + matrix[new_x][new_y]:
